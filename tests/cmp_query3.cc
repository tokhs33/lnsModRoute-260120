...
void queryCostValhallaWorker(
    const std::string& routePath,
    const size_t routeTasks,
    const size_t baseVehicle,
    const size_t nodeCount,
    CTaskQueue<std::shared_ptr<CTaskValhalla>>& tasks,
    std::vector<int64_t>& distMatrix,
    std::vector<int64_t>& timeMatrix)
{
    std::list<std::future<std::shared_ptr<CTaskValhalla>>> futures;

    for (size_t i = 0; i < routeTasks; i++) {
        auto task = tasks.pop();
        if (task == nullptr) {
            break;
        }
    ...
    bool showLog)
{
    ...
    size_t threadNum = 2;
    size_t threadTasks = routeTasks / threadNum;
    std::vector<std::future<void>> futures;
    for (size_t i = 0; i < threadNum; i++) {
        futures.emplace_back(
            pool.enqueue(
                queryCostValhallaWorker,
                routePath, threadTasks, baseVehicle, nodeCount, std::ref(tasks), std::ref(distMatrix), std::ref(timeMatrix)));
    }
    for (auto& f: futures) {
        f.get();
    }
    ...
    bool showLog)
{
    ...
    for (size_t s = 0; s < sourceCount; s += VALHALLA_MAX_LOCATIONS) {
        ...
        for (size_t d = 0; d < destinationCount; d += VALHALLA_MAX_LOCATIONS) {
            ...
            tasks.push(std::make_shared<CTaskValhalla>(sub_sources, sub_destinations, q_body));
        }
    }
}
    ...
    bool showLog)
{
    ...
    queryCostValhallaTask(routePath, routeTasks, 1, nodeCount, tasks, distMatrix, timeMatrix, pool, showLog);
...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        queryCostValhallaNotInCache(modRequest, routePath, nRouteTasks, nodeCount, changed, distMatrix, timeMatrix, pool, true);
