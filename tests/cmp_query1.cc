    ...
    bool showLog)
{
    ...
    for (size_t i = 0; i < runSize; i++) {
        ...
        futures.push_back(std::async(std::launch::async,
            ...
            routePath, task->m_body, baseVehicle, nodeCount, task->m_sources, task->m_destinations, std::ref(distMatrix), std::ref(timeMatrix)));
        tasks.pop_front();
    }
    ...
    bool showLog)
{
    ...
    for (size_t s = 0; s < nodeCount; s += VALHALLA_MAX_LOCATIONS) {
        ...
        std::string q_source = makeValhallaIndexLocs("sources", locs, sources);
        for (size_t d = 0; d < nodeCount; d += VALHALLA_MAX_LOCATIONS) {
            ...
            std::string q_dest = makeValhallaIndexLocs("targets", locs, destinations);
            std::string q_body = "{" + q_source + "," + q_dest + ",\"costing\":\"auto\"}";
            queryCostValhallaBody(routePath, q_body, 1, nodeCount, sources, destinations, distMatrix, timeMatrix);
        }
    }
